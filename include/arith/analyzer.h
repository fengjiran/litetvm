...
public:
    LITETVM_API ModularSet(int64_t coeff, int64_t base);
...
public:
     ...
     * \return the result of the analysis.
     */
    LITETVM_API ModularSet operator()(const PrimExpr& expr);
    /*!
     * \brief Update constant int bound information of var.
     *
     * \param var The variable of interest.
     ...
     * \param allow_override whether we allow override of existing information.
     */
    LITETVM_API void Update(const Var& var, const ModularSet& info, bool allow_override = false);

private:
    friend class Analyzer;
    friend class ConstraintContext;
    explicit ModularSetAnalyzer(Analyzer* parent);
    LITETVM_API ~ModularSetAnalyzer();
    /*!
     * \brief Update the internal state to enter constraint.
     * \param constraint A constraint expression.
     *
...
public:
   ...
   * \return the result of the analysis.
   */
     LITETVM_API PrimExpr operator()(const PrimExpr& expr);
   ...
   * \param allow_override Whether we allow override of existing information.
   */
     LITETVM_API void Update(const Var& var, const PrimExpr& new_expr, bool allow_override = false);
   ...
   * \return an exit function that must be called to cleanup the constraint can be nullptr.
   */
     LITETVM_API std::function<void()> EnterConstraint(const PrimExpr& constraint);
   ...
   * be enabled.
   */
     LITETVM_API void SetEnabledExtensions(Extension flags);

    /*! \brief Return the currently enabled extensions */
     LITETVM_API Extension GetEnabledExtensions() const;

    /*! \brief Return the statistics counters */
     LITETVM_API ObjectRef GetStatsCounters() const;

    /*! \brief Reset the statistics counters */
     LITETVM_API void ResetStatsCounters();
   ...
   */
     LITETVM_API void SetMaximumRewriteSteps(int64_t maximum);
...
private:
    ...
    explicit CanonicalSimplifier(Analyzer* parent);
     LITETVM_API ~CanonicalSimplifier();
    class Impl;
    /*! \brief Internal impl */
    Impl* impl_;
};
...
public:
   ...
   */
     LITETVM_API CompareResult TryCompare(const PrimExpr& lhs, const PrimExpr& rhs,
   ...
   */
     LITETVM_API void Bind(const Var& var, const PrimExpr& expr, bool allow_override = false);
...
private:
    ...
    TransitiveComparisonAnalyzer();
     LITETVM_API ~TransitiveComparisonAnalyzer();
    class Impl;
    /*! \brief Internal impl */
...
public:
     ...
     * \return the result of the analysis.
     */
     LITETVM_API IntSet operator()(const PrimExpr& expr, const Map<Var, IntSet>& dom_map);
     ...
     * \return the result of the analysis.
     */
     LITETVM_API IntSet operator()(const PrimExpr& expr);
     ...
     * \param allow_override whether we allow override of existing information.
     */
     LITETVM_API void Update(const Var& var, const IntSet& new_interval_set, bool allow_override = false);
     ...
     * \param allow_override whether we allow override of existing information.
     */
     LITETVM_API void Bind(const Var& var, const Range& new_range, bool allow_override = false);
