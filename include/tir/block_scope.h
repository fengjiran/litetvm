...
#ifndef LITETVM_TIR_BLOCK_SCOPE_H
#define LITETVM_TIR_BLOCK_SCOPE_H
...
public:
   ...
   */
    LITETVM_API explicit StmtSRef(const StmtNode* stmt, StmtSRefNode* parent, int64_t seq_index);
...
public:
   ...
   */
    LITETVM_API static StmtSRef InlineMark();
    /*!
   * \return A special StmtSRef, which doesn't point to any stmt in the AST,
   ...
   *   compute-at-impl(block, loop_sref)    otherwise
   */
    LITETVM_API static StmtSRef RootMark();
};
...
public:
    /*! \brief Constructor */
    LITETVM_API explicit Dependency(StmtSRef src, StmtSRef dst, DepKind kind);
...
public:
   ...
   */
    LITETVM_API Array<Dependency> GetDepsBySrc(const StmtSRef& src) const;
    /*!
   * \brief Get all dependencies whose `dst` equals `dst`
   * \param dst The queried block
   * \return The dependencies
   */
    LITETVM_API Array<Dependency> GetDepsByDst(const StmtSRef& dst) const;
};
...
public:
   ...
   */
    LITETVM_API explicit BlockScope(const Array<StmtSRef>& child_block_srefs);
...
#endif//LITETVM_TIR_BLOCK_SCOPE_H
