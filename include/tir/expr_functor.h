...
void VisitArray(const Array<T>& arr, F fvisit) {
...
template<typename T, typename F>
Array<T> MutateArray(Array<T> arr, F fmutate) {
 ...
 */
class LITETVM_API ExprMutator : protected ExprFunctor<PrimExpr(const PrimExpr&)> {
public:
    using ExprFunctor::operator();
