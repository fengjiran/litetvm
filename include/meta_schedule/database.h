...
#ifndef LITETVM_META_SCHEDULE_DATABASE_H
#define LITETVM_META_SCHEDULE_DATABASE_H

#include "ir/expr.h"
#include "ir/module.h"
#include "meta_schedule/arg_info.h"
...
public:
   ...
   */
    LITETVM_API explicit Workload(IRModule mod);
    /*!
   * \brief Constructor of Workload.
   * \param mod The workload's IRModule.
   * \param shash The workload's structural hash.
   */
    LITETVM_API explicit Workload(IRModule mod, THashCode shash);
    /*!
   * \brief Create a workload from a json object.
   * \param json_obj The json object.
   * \return The created workload.
   */
    LITETVM_API static Workload FromJSON(const ObjectRef& json_obj);
 ...
 * \sa TuningRecordNode
 */
class TuningRecord : public ObjectRef {
public:
    /*!
  ...
  */
    LITETVM_API explicit TuningRecord(tir::Trace trace, Workload workload,
   ...
   */
    LITETVM_API static TuningRecord FromJSON(const ObjectRef& json_obj, const Workload& workload);
...
public:
   ...
   * \return Whether the database has the given workload.
   */
    using FHasWorkload = TypedPackedFunc<bool(const IRModule&)>;
    /*!
   * \brief The function type of `CommitWorkload` method.
   * \param mod The IRModule to be searched for or added.
   * \return The workload corresponding to the given IRModule.
   */
    using FCommitWorkload = TypedPackedFunc<Workload(const IRModule&)>;
    /*!
   ...
   * \return An array of top K tuning records for the given workload.
   */
    using FGetTopK = TypedPackedFunc<Array<TuningRecord>(const Workload&, int)>;
    /*!
   * \brief The function type of `GetAllTuningRecords` method.
   * \return An Array of all the tuning records in the database.
   */
    using FGetAllTuningRecords = TypedPackedFunc<Array<TuningRecord>()>;
    /*!
   ...
   * \return The best record of the given workload; NullOpt if not found.
   */
    using FQueryTuningRecord = TypedPackedFunc<Optional<TuningRecord>(
   ...
   * \return The schedule in the best schedule of the given workload; NullOpt if not found.
   */
    using FQuerySchedule = TypedPackedFunc<Optional<tir::Schedule>(
            const IRModule&, const Target&, const String&)>;
    /*!
   ...
   */
    using FQueryIRModule =
            TypedPackedFunc<Optional<IRModule>(const IRModule&, const Target&, const String&)>;
    /*!
   * \brief The function type of `Size` method.
   * \return The size of the database.
   */
    using FSize = TypedPackedFunc<int64_t()>;
    ...
    bool HasWorkload(const IRModule& mod) final {
        CHECK(f_has_workload != nullptr) << "PyDatabase's HasWorkload method not implemented!";
    ...
    Array<TuningRecord> GetAllTuningRecords() final {
        CHECK(f_get_all_tuning_records != nullptr)
...
public:
   ...
   * \param mod_eq_name A string to specify the module equality testing and hashing method.
   */
    LITETVM_API static Database MemoryDatabase(String mod_eq_name = "structural");
    /*!
   ...
   * \param mod_eq_name A string to specify the module equality testing and hashing method.
   */
    LITETVM_API static Database ScheduleFnDatabase(
   ...
   * \param mod_eq_name A string to specify the module equality testing and hashing method.
   */
    LITETVM_API static Database JSONDatabase(String path_workload, String path_tuning_record,
   ...
   */
    LITETVM_API static Database UnionDatabase(Array<Database, void> databases);
    /*!
   * \brief A database composed of multiple databases, allowing users to guide IR rewriting using
   ...
   */
    LITETVM_API static Database OrderedUnionDatabase(Array<Database, void> databases);
    /*!
   ...
   */
    LITETVM_API static Database PyDatabase(PyDatabaseNode::FHasWorkload f_has_workload,
...
#endif//LITETVM_META_SCHEDULE_DATABASE_H
