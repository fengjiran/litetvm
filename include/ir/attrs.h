...
public:
   ...
   */
    LITETVM_API explicit DictAttrs(Map<String, ObjectRef> dict = {});
 ...
 * \tparam TAttrs the type to be created.
 * \return AN instance that will represent None.
 */
...
void SetIntValue(T* ptr, const TVMArgValue& val) {
    if (val.type_code() == static_cast<int>(DLDataTypeCode::kDLInt)) {
...
inline void SetValue<double>(double* ptr, const TVMArgValue& val) {
    if (val.type_code() == static_cast<int>(DLDataTypeCode::kDLFloat) ||
        val.type_code() == static_cast<int>(DLDataTypeCode::kDLInt)) {
...
public:
    ...
    bool SEqualReduce(const DerivedType* other, SEqualReducer equal) const {
        ...
        detail::AttrsSEqualVisitor visitor(pself, other, equal);
        self()->_tvm_VisitAttrs(visitor);
        return visitor.result_;
    }

    void SHashReduce(SHashReducer hash_reducer) const {
        detail::AttrsSHashVisitor visitor(hash_reducer);
        self()->_tvm_VisitAttrs(visitor);
    }
