...
#ifndef LITETVM_IR_NAME_SUPPLY_H
#define LITETVM_IR_NAME_SUPPLY_H
...
public:
   ...
   */
    LITETVM_API explicit NameSupply(const String& prefix = "",
                                std::unordered_map<std::string, int> name_map = {});
   ...
   */
    ...
    LITETVM_API explicit NameSupply(Iter begin, Iter end, Lambda f)
...
private:
    ...
    static std::unordered_map<std::string, int> GetNameMap(Iter begin, Iter end, Lambda f) {
        ...
        for (auto it = begin; it != end; ++it) {
            ...
            CHECK(prefix.size() > 0 && std::isalpha(prefix[0])) << "Invalid variable name: " << name;
...
#endif//LITETVM_IR_NAME_SUPPLY_H
