...
#ifndef LITETVM_RELAX_TUNING_API_H
...
class ChoiceNode : public Object {
public:
    ...
    const PackedFunc GetConstrFunc() {
        const auto* constr_func = runtime::RegistryManager::Global().Get(constr_func_key);
        CHECK(constr_func != nullptr) << "constr_func_key is not registered: " << constr_func_key;
        return *constr_func;
    }

    /*! \brief Getter for transform_func. */
    const PackedFunc GetTransformFunc() {
        auto* transform_func = runtime::RegistryManager::Global().Get(transform_func_key);
        CHECK(transform_func != nullptr)
...
class Choice : public ObjectRef {
public:
    LITETVM_API explicit Choice(String transform_func_key, Array<ObjectRef> transform_func_args,
                            String constr_func_key, Array<ObjectRef> constr_func_args);
    /*! \brief Deserialize JSON-style object into Choice */
    LITETVM_API static Choice FromJSON(const ObjectRef& json_obj);
...
public:
   ...
   */
    IRModule Apply(IRModule mod, String decision) {
        CHECK(IsValidDecision(decision)) << "Invalid choice for this knob: " << decision;
...
class Knob : public ObjectRef {
public:
    LITETVM_API explicit Knob(String name, Map<String, Choice> choices);
    /*! \brief Deserialize JSON-style object into Knob */
    LITETVM_API static Knob FromJSON(const ObjectRef& json_obj);
...
public:
   ...
   */
    LITETVM_API explicit Trace(IRModule in_mod, Array<Knob> knobs, Array<String> decisions);
    /*! \brief Deserialize JSON-style object into Trace */
    LITETVM_API static Trace FromJSON(const ObjectRef& json_obj);
    TVM_DEFINE_MUTABLE_NOTNULLABLE_OBJECT_REF_METHODS(Trace, ObjectRef, TraceNode);
};

/*! \brief The class of tuning records. */
class TuningRecordNode : public Object {
public:
...
public:
  ...
  */
    LITETVM_API explicit TuningRecord(Trace trace, Optional<Array<FloatImm>> run_secs);
    /*!
   * \brief Create a tuning record from a json object.
   * \param json_obj The json object.
   * \return The tuning record created.
   */
    LITETVM_API static TuningRecord FromJSON(const ObjectRef& json_obj);
...
struct WorkloadEqual {
    bool operator()(const meta_schedule::Workload& a, const meta_schedule::Workload& b) const {
        return a->shash == b->shash && StructuralEqual()(a->mod, b->mod);
    }
};
...
#endif//LITETVM_RELAX_TUNING_API_H
