//
// Created by richard on 5/11/25.
//

#ifndef LITETVM_RELAX_ANALYSIS_H
#define LITETVM_RELAX_ANALYSIS_H

#include "arith/analyzer.h"
#include "ir/module.h"
#include "relax/expr.h"
#include "relax/struct_info.h"
#include "relax/op_attr_types.h"
 ...
 */
LITETVM_API bool CanProveShapeEqual(const Array<PrimExpr>& lhs, const Array<PrimExpr>& rhs,
                                arith::Analyzer* ana);
 ...
 *       two shapes equals to each other during runtime.
 */
LITETVM_API bool CanProveShapeEqual(const Expr& lhs, const Expr& rhs, arith::Analyzer* ana);
 ...
 */
LITETVM_API Type GetStaticType(const StructInfo& info);
 ...
 */
LITETVM_API StructInfo StructInfoFromType(const Type& type);
 ...
 */
LITETVM_API StructInfo DeriveCallRetStructInfo(const FuncStructInfo& finfo, const Call& call,
 ...
 */
LITETVM_API StructInfo
EraseToWellDefined(const StructInfo& info,
                   const std::function<Optional<PrimExpr>(const tir::Var& var)>& f_shape_var_map = nullptr,
                   const std::function<Optional<Expr>(const Var& var)>& f_var_map = nullptr,
 ...
 * \return the corresponding erased struct info.
 */
LITETVM_API StructInfo EraseToWellDefined(const StructInfo& info, Map<tir::Var, PrimExpr> shape_var_map,
 ...
 * \sa BaseCheckResult
 */
LITETVM_API BaseCheckResult StructInfoBaseCheck(const StructInfo& base, const StructInfo& derived,
                                            arith::Analyzer* ana = nullptr);
 ...
 * \return Whether the relation holds.
 */
LITETVM_API bool IsBaseOf(const StructInfo& base, const StructInfo& derived,
                      arith::Analyzer* ana = nullptr);
 ...
 * \sa BaseCheckResult
 */
LITETVM_API PrimExpr StructInfoBaseCheckPrecondition(const StructInfo& base, const StructInfo& derived);
 ...
 */
LITETVM_API StructInfo StructInfoLCA(const StructInfo& lhs, const StructInfo& rhs,
                                 arith::Analyzer* ana = nullptr);
 ...
 */
LITETVM_API Array<tir::Var> TIRVarsInStructInfo(const StructInfo& sinfo);
 ...
 *   order of occurrence.
 */
LITETVM_API Array<tir::Var> DefinableTIRVarsInStructInfo(const StructInfo& sinfo);
 ...
 */
LITETVM_API Array<PrimExpr> CollectNonNegativeExpressions(const StructInfo& sinfo);
 ...
 */
LITETVM_API Array<tir::Var> DefinedSymbolicVars(const Expr& expr);
 ...
 */
LITETVM_API Array<tir::Var> FreeSymbolicVars(const Expr& expr);
//-----------------------------------
// General IR analysis
//-----------------------------------
/*!
 ...
 */
LITETVM_API Array<Var> BoundVars(const Expr& expr);
 ...
 */
LITETVM_API Array<Var> FreeVars(const Expr& expr);
 ...
 */
LITETVM_API Array<Var> AllVars(const Expr& expr);
 ...
 */
LITETVM_API Array<GlobalVar> AllGlobalVars(const Expr& expr);
 ...
 */
LITETVM_API Array<Array<GlobalVar>> DetectRecursion(const IRModule& m);
 ...
 */
LITETVM_API Map<Var, Expr> AnalyzeVar2Value(const IRModule& m);
 ...
 * \return Var -> Value (Expr)
 */
LITETVM_API Map<Var, Expr> AnalyzeVar2Value(const Expr& expr);
 ...
 * \return Var -> Value (Expr)
 */
LITETVM_API Map<Var, Expr> AnalyzeVar2Value(const DataflowBlock& dfb);
 ...
 */
LITETVM_API Map<String, Array<Binding>> NameToBinding(const Function& fn);
 ...
 */
LITETVM_API Map<Var, Array<Var>> DataflowBlockUseDef(const DataflowBlock& dfb);
 ...
 */
LITETVM_API Expr RemoveAllUnused(Expr expr);
 ...
 */
LITETVM_API OpPatternKind AnalyzeOpPatternKind(const tir::PrimFunc& func);
 ...
 */
LITETVM_API bool HasReshapePattern(const tir::PrimFunc& func);
 ...
 */
LITETVM_API Optional<Expr> FindImpureCall(const Expr& expr,
 ...
 *   an impure call--it only does if the nested function is *later called*.
 */
LITETVM_API bool ContainsImpureCall(const Expr& expr,
                                const Optional<Expr>& own_name = Optional<Expr>(nullptr));
 ...
 */
LITETVM_API bool WellFormed(Variant<IRModule, Function> obj, bool check_struct_info = true);
...
LITETVM_API Map<tir::Block, Map<ObjectRef, tir::IndexMap>> SuggestLayoutTransforms(
 ...
 */
LITETVM_API Array<Var> ComputableAtCompileTime(const Function& func);

}// namespace relax
}// namespace litetvm

#endif//LITETVM_RELAX_ANALYSIS_H
