...
public:
     ...
     * \param span The source span of the expression.
     */
    LITETVM_API explicit Tuple(Array<Expr> fields);
     ...
     * \param span The source span of the expression.
     */
    template<typename RelaxExpr,
             typename = std::enable_if_t<std::is_base_of_v<Expr, RelaxExpr>>>
    LITETVM_API explicit Tuple(Array<RelaxExpr> fields)
        : Tuple(fields.Map([](const RelaxExpr& expr) -> Expr { return expr; })) {}
...
public:
     ...
     * \param span The source span of the expression.
     */
    LITETVM_API TupleGetItem(Expr tuple, int index);
...
public:
    LITETVM_API explicit ShapeExpr(Array<PrimExpr> values);
...
public:
   ...
   * \param span The source span of the expression.
   */
    LITETVM_API explicit PrimValue(PrimExpr value);
   ...
   */
    LITETVM_API static PrimValue Int64(int64_t value);
...
public:
   ...
   * \param span The source span of the expression.
   */
    LITETVM_API explicit StringImm(String value);
...
public:
   ...
   * \param span The source span of the expression.
   */
    LITETVM_API explicit DataTypeImm(DataType value);
...
public:
    LITETVM_API explicit MatchCast(Var var, Expr value, StructInfo struct_info);
...
public:
    LITETVM_API explicit VarBinding(Var var, Expr value);
...
public:
    LITETVM_API explicit BindingBlock(Array<Binding> bindings);
...
public:
    LITETVM_API explicit DataflowBlock(Array<Binding> bindings);
...
public:
   ...
   */
    LITETVM_API SeqExpr(Expr body);// NOLINT(*)

    LITETVM_API explicit SeqExpr(Array<BindingBlock> blocks, Expr body);
...
public:
   ...
   * \param span The source span of the expression.
   */
    LITETVM_API If(Expr cond, Expr true_branch, Expr false_branch);
...
public:
   ...
   * \param span The source span of the expression.
   */
    LITETVM_API explicit Function(Array<Var> params, Expr body, Optional<StructInfo> ret_struct_info,
   ...
   */
    LITETVM_API static Function CreateEmpty(Array<Var> params, StructInfo ret_struct_info,
                                            bool is_pure = true, DictAttrs attrs = DictAttrs());
...
public:
    LITETVM_API ExternFunc(String global_symbol);
    LITETVM_API ExternFunc(String global_symbol, StructInfo struct_info);
 ...
 */
LITETVM_API Expr GetShapeOf(const Expr& expr);
 ...
 */
template<>
struct std::hash<litetvm::relax::Var> {
    std::size_t operator()(const litetvm::relax::Var& var) const {
        return litetvm::runtime::ObjectPtrHash()(var);
    }
};

template<>
struct std::equal_to<litetvm::relax::Var> {
    bool operator()(const litetvm::relax::Var& var_a, const litetvm::relax::Var& var_b) const {
