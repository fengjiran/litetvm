...
public:
    ...
    LITETVM_API PrimStructInfo(DataType dtype);

    /* Construct a PrimStructInfo with a known value */
    LITETVM_API PrimStructInfo(PrimExpr value);
...
public:
     ...
     * \param span The span of the AST.
     */
    LITETVM_API ShapeStructInfo(int ndim);
...
public:
   ...
   */
    LITETVM_API TensorStructInfo(Expr shape, DataType dtype, Optional<VDevice> vdevice = NullOpt);
   ...
   * \param span The span of the AST.
   */
    LITETVM_API TensorStructInfo(DataType dtype, int ndim, Optional<VDevice> vdevice = NullOpt);
...
public:
     ...
     * \param span The span of the AST.
     */
    LITETVM_API TupleStructInfo(Array<StructInfo> fields);
...
public:
   ...
   */
    LITETVM_API FuncStructInfo(Array<StructInfo> params, StructInfo ret, bool purity = true);
   ...
   */
    LITETVM_API static FuncStructInfo OpaqueFunc(StructInfoDeriveFunc derive_func, bool purity = false);
   ...
   * \note Defaults to an derive func that always return ObjectStructInfo if not specified.
   */
    LITETVM_API static FuncStructInfo OpaqueFunc(StructInfo ret = ObjectStructInfo(), bool purity = false);
...
inline StructInfo GetStructInfo(const Expr& expr) {
    ...
    CHECK(ptr) << "The struct_info is not populated, check if you have normalized the expr";
 ...
 */
LITETVM_API void UpdateStructInfo(Expr expr, StructInfo struct_info);
