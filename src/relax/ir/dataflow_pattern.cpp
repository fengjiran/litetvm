...
TuplePattern::TuplePattern(Array<DFPattern> fields) {
    ObjectPtr<TuplePatternNode> n = make_object<TuplePatternNode>();
    n->fields = std::move(fields);
    data_ = std::move(n);
}
TVM_REGISTER_GLOBAL("relax.dpl.TuplePattern").set_body_typed([](Array<DFPattern> fields) {
...
UnorderedTuplePattern::UnorderedTuplePattern(Array<DFPattern> fields) {
...
TVM_REGISTER_GLOBAL("relax.dpl.UnorderedTuplePattern")
        .set_body_typed([](Array<DFPattern> fields) { return UnorderedTuplePattern(fields); });
...
void PatternContext::ExitWithScope() const {
    CHECK(pattern_ctx_stack().top().same_as(*this));
...
PatternSeq::PatternSeq(Array<DFPattern> patterns, bool only_used_by) {
    CHECK_GE(patterns.size(), 1) << "PatternSeq must have at least one pattern";
