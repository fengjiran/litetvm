...
public:
    explicit ExprBinder(const Map<Var, Expr>& args_map,
...
private:
    ...
    const Map<Var, Expr>& args_map_;
    const Map<tir::Var, PrimExpr>& symbolic_var_map_;
};
 ...
 */
Expr Bind(const Expr& expr, const Map<Var, Expr>& binds,
          const Map<tir::Var, PrimExpr>& symbolic_var_map) {
    return ExprBinder(binds, symbolic_var_map).VisitExpr(expr);
}

StructInfo Bind(const StructInfo& sinfo, const Map<tir::Var, PrimExpr>& symbolic_var_map) {
    return ExprBinder({}, symbolic_var_map).VisitExprDepStructInfoField(sinfo);
}

Map<tir::Var, PrimExpr> InferSymbolicVarMap(const Map<Var, Expr>& relax_var_remap, arith::Analyzer* analyzer) {
...
bool IsImpureCall(const Call& call) {
    if (auto op_ptr = call->op.as<OpNode>()) {
        ...
        CHECK(purity_map.count(op)) << "Cannot find the registered purity of this op: " << op->name;
...
Expr GetBoundValue(const Binding& b) {
    ...
    if (auto* match_binding = b.as<MatchCastNode>()) {
...
Function CopyWithNewVars(Function func) {
    return FunctionCopier().Copy(std::move(func));
}
