...
Array<Var> FreeVars(const Expr& expr) {
    return VarVisitor().Free(expr);
}

Array<Var> BoundVars(const Expr& expr) {
    return VarVisitor().Bound(expr);
}

Array<Var> AllVars(const Expr& expr) {
...
Optional<Expr> FindImpureCall(const Expr& expr, const Optional<Expr>& own_name) {
    ...
    if (own_name) {
        CHECK(own_name.value().as<VarNode>() || own_name.value().as<GlobalVarNode>())
