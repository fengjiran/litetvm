...
std::string TargetInternal::JoinString(const std::vector<std::string>& array, char separator) {
    std::string result;
    CHECK(separator != quote && separator != escape)
...
std::vector<std::string> TargetInternal::SplitString(const std::string& str, char separator) {
    ...
    CHECK(!pos_quoted) << "Mismatched quotes '' in string";
...
ObjectRef TargetInternal::ParseType(const std::string& str,
    ...
    if (info.type_index == Target::ContainerType::_GetOrAllocRuntimeTypeIndex()) {
        // Parsing target
        return Target(FromString(interp_str));
    }
    ...
    if (info.type_index == ArrayNode::_GetOrAllocRuntimeTypeIndex()) {
        ...
        for (const std::string& substr: SplitString(interp_str, ',')) {
            try {
                ObjectRef parsed = ParseType(substr, *info.key);
...
ObjectRef TargetInternal::ParseType(const ObjectRef& obj,
    ...
    if (info.type_index == String::ContainerType::RuntimeTypeIndex()) {
        // Parsing string
        return GetRef<String>(ObjTypeCheck<StringObj>(obj, "String"));
    }

    if (info.type_index == Target::ContainerType::RuntimeTypeIndex()) {
        // Parsing target
        if (auto opt = obj.as<Target>()) {
            return opt.value();
        }
        if (auto str = obj.as<String>()) {
            return Target(FromString(str.value()));
        }
        if (const auto* ptr = obj.as<MapNode>()) {
            ...
            return Target(FromConfig({config.begin(), config.end()}));
        }
    ...
    if (info.type_index == ArrayNode::_GetOrAllocRuntimeTypeIndex()) {
        ...
        for (const ObjectRef& e: *array) {
            try {
                result.push_back(ParseType(e, *info.key));
            } catch (const Error& e) {
                std::string index = '[' + std::to_string(result.size()) + ']';
                throw Error(index + e.what());
            }
    ...
    if (info.type_index == MapNode::_GetOrAllocRuntimeTypeIndex()) {
        ...
        for (const auto& kv: *map) {
            ObjectRef key, val;
            try {
                key = ParseType(kv.first, *info.key);
            } catch (const Error& e) {
                std::ostringstream os;
                os << "'s key \"" << key << "\"" << e.what();
                throw Error(os.str());
            ...
            try {
                val = ParseType(kv.second, *info.val);
            } catch (const Error& e) {
                std::ostringstream os;
                os << "[\"" << key << "\"]" << e.what();
                throw Error(os.str());
...
std::string TargetInternal::StringifyAtomicType(const ObjectRef& obj) {
    ...
    if (const auto* p = obj.as<runtime::Bool::ContainerType>()) {
        return std::to_string(p->value);
    }
...
//             TargetKind::GetAttrMap<Bool>(attr::kIsExternalCodegen);
//     TargetKindAttrMap<litetvm::transform::Pass> relay_to_tir_map =
//             TargetKind::GetAttrMap<litetvm::transform::Pass>(attr::kRelayToTIR);
...
Optional<Target> TargetNode::GetHost() const { return this->host.as<Target>().value(); }
...
void Target::ExitWithScope() const {
    TVMTargetThreadLocalEntry* entry = TVMTargetThreadLocalStore::Get();
...
void TargetInternal::ConstructorDispatcher(TVMArgs args, TVMRetValue* rv) {
    if (args.num_args == 1) {
        ...
        } else if (arg.type_code() == static_cast<int>(TVMArgTypeCode::kTVMObjectHandle)) {
            ObjectRef obj(arg);
...
ObjectPtr<Object> TargetInternal::FromString(const String& tag_or_config_or_target_str) {
    ...
    if (!tag_or_config_or_target_str.empty() && tag_or_config_or_target_str.data()[0] == '{') {
        return FromConfigString(tag_or_config_or_target_str);
    }
    return FromRawString(tag_or_config_or_target_str);
}

ObjectPtr<Object> TargetInternal::FromConfigString(const String& config_str) {
    const auto* loader = runtime::RegistryManager::Global().Get("target._load_config_dict");
    ...
    return FromConfig({config.value().begin(), config.value().end()});
}

ObjectPtr<Object> TargetInternal::FromRawString(const String& target_str) {
    CHECK_GT(target_str.length(), 0) << "Cannot parse empty target string";
    ...
    for (size_t iter = 1, end = options.size(); iter < end;) {
        ...
        try {
            ...
            config[key] = ParseType(value, FindTypeInfo(kind, key));
        } catch (const Error& e) {
...
ObjectPtr<Object> TargetInternal::FromConfig(Map<String, ObjectRef> config) {
    ...
    CHECK(!config.count(kFeatures)) << "Target Features should be generated by Target parser";

    // parse 'kind'
    if (config.count(kKind)) {
        if (auto kind = config[kKind].as<String>()) {
            target->kind = GetTargetKind(kind.value());
            CHECK(!(target->kind->preprocessor != nullptr && target->kind->target_parser != nullptr))
...
std::unordered_map<String, ObjectRef> TargetInternal::QueryDevice(int device_id,
    ...
    TVMRetValue ret;
    api->GetAttr(device, runtime::DeviceAttrKind::kExist, &ret);
    bool device_exists(ret);
    if (!device_exists) {
        CHECK(device_exists) << "Requested reading the parameters for " << target->kind->name
