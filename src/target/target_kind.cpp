 ...
 * then returns integer 20 after the `prefix`
...
std::string ExtractStringWithPrefix(const std::string& str, const std::string& prefix) {
    ...
    int n = prefix.size();
    int pos = n;
    while (pos < m && std::isalnum(str[pos])) {
        ++pos;
    }
    return str.substr(n, pos - n);
}
...
static bool DetectDeviceFlag(Device device, runtime::DeviceAttrKind flag, TVMRetValue* val) {
    ...
    api->GetAttr(device, runtime::DeviceAttrKind::kExist, val);
    int exists(*val);
    if (!exists) {
        return false;
    }
...
void CheckOrSetAttr(Map<String, ObjectRef>* attrs, const String& name, const String& value) {
    ...
    if (iter == attrs->end()) {
        ...
        CHECK(str && str.value() == value) << "ValueError: Expects \"" << name << "\" to be \""
...
TargetJSON UpdateCUDAAttrs(TargetJSON target) {
    ...
    if (target.count("arch")) {
        ...
        CHECK(support::StartsWith(archStr, "sm_"))
                << "ValueError: CUDA target gets an invalid CUDA arch: -arch=" << archStr;
    } else {
        // Use the compute version of the first CUDA GPU instead
        int archInt;
        TVMRetValue version;
        if (!DetectDeviceFlag({DLDeviceType::kDLCUDA, 0}, runtime::DeviceAttrKind::kComputeVersion, &version)) {
...
TargetJSON UpdateNVPTXAttrs(TargetJSON target) {
    ...
    if (target.count("mcpu")) {
        ...
        CHECK(support::StartsWith(mcpu, "sm_"))
...
TargetJSON UpdateROCmAttrs(TargetJSON target) {
    ...
    if (target.count("mcpu")) {
        ...
        CHECK(!arch.empty()) << "ValueError: ROCm target gets an invalid GFX version: -mcpu=" << mcpu;
    } else {
        TVMRetValue val;
        if (const auto* f_get_rocm_arch = RegistryManager::Global().Get("tvm_callback_rocm_get_arch")) {
    ...
    TVMRetValue val;
    int version;
    if (!DetectDeviceFlag({DLDeviceType::kDLROCM, 0}, runtime::DeviceAttrKind::kApiVersion, &val)) {
...
TVM_REGISTER_TARGET_KIND("llvm", static_cast<int>(DLDeviceType::kDLCPU))
        ...
        .set_attr<runtime::Bool>(attr::kIsExternalCodegen, runtime::Bool(false))
        .set_target_parser(target::parsers::cpu::ParseTarget);
...
TVM_REGISTER_TARGET_KIND("c", static_cast<int>(DLDeviceType::kDLCPU))
        .add_attr_option<String>("mcpu")
        .add_attr_option<String>("march")
        .add_attr_option<runtime::Int>("workspace-byte-alignment")
        .add_attr_option<runtime::Int>("constants-byte-alignment")
        .set_default_keys({"cpu"})
        .set_target_parser(litetvm::target::parsers::cpu::ParseTarget);

TVM_REGISTER_TARGET_KIND("cuda", static_cast<int>(DLDeviceType::kDLCUDA))
        .add_attr_option<String>("mcpu")
...
TVM_REGISTER_TARGET_KIND("nvptx", static_cast<int>(DLDeviceType::kDLCUDA))
        .add_attr_option<String>("mcpu")
        .add_attr_option<String>("mtriple")
...
TVM_REGISTER_TARGET_KIND("rocm", static_cast<int>(DLDeviceType::kDLROCM))
        .add_attr_option<String>("mcpu")
        .add_attr_option<String>("mtriple")
        .add_attr_option<Array<String>>("mattr")
...
TVM_REGISTER_TARGET_KIND("opencl", (int) DLDeviceType::kDLOpenCL)
        .add_attr_option<runtime::Int>("max_threads_per_block", runtime::Int(256))
...
TVM_REGISTER_TARGET_KIND("metal", static_cast<int>(DLDeviceType::kDLMetal))
        .add_attr_option<runtime::Int>("max_num_threads", runtime::Int(256))
        .add_attr_option<runtime::Int>("max_threads_per_block", runtime::Int(256))
...
TVM_REGISTER_TARGET_KIND("vulkan", static_cast<int>(DLDeviceType::kDLVulkan))
        .add_attr_option<Array<String>>("mattr")
        // Feature support
...
TVM_REGISTER_TARGET_KIND("webgpu", static_cast<int>(DLDeviceType::kDLWebGPU))
        .add_attr_option<runtime::Int>("max_num_threads", runtime::Int(256))
        .set_default_keys({"webgpu", "gpu"});

TVM_REGISTER_TARGET_KIND("sdaccel", static_cast<int>(DLDeviceType::kDLOpenCL))// line break
        .set_default_keys({"sdaccel", "hls"});

TVM_REGISTER_TARGET_KIND("aocl", static_cast<int>(TVMDeviceExtType::kDLAOCL))// line break
        .set_default_keys({"aocl", "hls"});

TVM_REGISTER_TARGET_KIND("aocl_sw_emu", (int) TVMDeviceExtType::kDLAOCL)// line break
        .set_default_keys({"aocl", "hls"});

TVM_REGISTER_TARGET_KIND("hexagon", static_cast<int>(DLDeviceType::kDLHexagon))
        .add_attr_option<Array<String>>("mattr")
        .add_attr_option<String>("mcpu")
        .add_attr_option<String>("mtriple")
...
TVM_REGISTER_TARGET_KIND("stackvm", static_cast<int>(DLDeviceType::kDLCPU))// line break
        .set_default_keys({"cpu"});

TVM_REGISTER_TARGET_KIND("ext_dev", static_cast<int>(DLDeviceType::kDLExtDev));

TVM_REGISTER_TARGET_KIND("hybrid", static_cast<int>(DLDeviceType::kDLCPU));

TVM_REGISTER_TARGET_KIND("composite", static_cast<int>(DLDeviceType::kDLCPU))// line break
        .add_attr_option<Array<Target>>("devices");

TVM_REGISTER_TARGET_KIND("test", static_cast<int>(DLDeviceType::kDLCPU))// line break
