...
IRModule::IRModule(Map<GlobalVar, BaseFunc> functions, DictAttrs attrs,
...
void IRModuleNode::AddUnchecked(const GlobalVar& var, const BaseFunc& func) {
    ...
    if (it != global_var_map_.end()) {
        ...
        CHECK(global_var_map_.count(var->name_hint) == 0) << "Duplicate global function name " << var;
    }
...
BaseFunc IRModuleNode::Lookup(const GlobalVar& var) const {
    ...
    CHECK(it != functions.end()) << "There is no definition of " << var;
    return (*it).second;
}
...
IRModule IRModuleNode::ShallowCopy() {
    return IRModule(this->functions, this->attrs, this->global_infos);
}
...
IRModule IRModule::FromExpr(const RelaxExpr& expr,
    ...
    if (auto func_node = expr.as<BaseFunc>()) {
        ...
        if (auto opt = func->GetAttr<String>(attr::kGlobalSymbol)) {
...
TVM_REGISTER_GLOBAL("ir.IRModule")
        .set_body_typed([](Map<GlobalVar, BaseFunc> funcs, ObjectRef attrs,
                           Map<String, Array<GlobalInfo>> global_infos) {
            auto dict_attrs = [&attrs]() {
                if (!attrs.defined()) {
                    return DictAttrs();
                } else if (auto* as_dict_attrs = attrs.as<DictAttrsNode>()) {
                    return GetRef<DictAttrs>(as_dict_attrs);
                } else if (attrs.as<MapNode>()) {
                    return DictAttrs(Downcast<Map<String, ObjectRef>>(attrs));
                } else {
...
TVM_REGISTER_GLOBAL("ir.Module_Add")
        .set_body_typed([](IRModule mod, GlobalVar var, ObjectRef val, bool update) -> IRModule {
            CHECK(val->IsInstance<RelaxExprNode>());
