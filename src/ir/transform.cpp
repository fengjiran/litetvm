...
public:
    ...
    void Legalize(Map<String, ObjectRef>* config) {
        ...
        for (auto [key, obj]: *config) {
            ...
            CHECK(obj.defined()) << "AttributeError: " << key << " is None";

            CHECK(info.legalization) << "AttributeError: "
                                     << "Config option \'" << key
...
IRModule Pass::operator()(IRModule mod, const PassContext& pass_ctx) const {
    ...
    if (pass_ctx->GetConfig<Bool>("testing.immutable_module", Bool(false)).value()) {
        ret = AssertImmutableModule(mod, node, pass_ctx);
    } else {
...
IRModule Pass::AssertImmutableModule(const IRModule& mod, const PassNode* node,
    ...
    size_t before_pass_hash = StructuralHash()(mod);
    ObjectPtr<Object> module_ptr = GetDataPtr<Object>(mod);
    IRModule copy_mod = IRModule(module_ptr);
    IRModule ret = node->operator()(mod, pass_ctx);
    size_t after_pass_hash = StructuralHash()(copy_mod);
    if (before_pass_hash != after_pass_hash) {
        ...
        // must be very low.
        DLOG(DFATAL) << "Immutable module has been modified in pass: " << node->Info()->name;
    }
    return std::move(ret);
}
...
public:
    ...
    void VisitAttrs(AttrVisitor* v) { v->Visit("pass_info", &pass_info); }
...
public:
    ModulePass(TypedPackedFunc<IRModule(IRModule, PassContext)> pass_func,
               PassInfo pass_info);

    TVM_DEFINE_OBJECT_REF_METHODS(ModulePass, Pass, ModulePassNode);
};

PassInfo::PassInfo(int opt_level, String name, Array<String> required,
                   bool traceable) {
...
ModulePass::ModulePass(TypedPackedFunc<IRModule(IRModule, PassContext)> pass_func,
                       PassInfo pass_info) {
...
IRModule ModulePassNode::operator()(IRModule mod, const PassContext& pass_ctx) const {
    ...
    CHECK(mod.defined()) << "The return value of a module pass must be set.";
...
Pass GetPass(const String& pass_name) {
    ...
    const PackedFunc* f = nullptr;
    if (pass_name.operator std::string().find("transform.") != std::string::npos) {
        f = RegistryManager::Global().Get(pass_name);
    } else if ((f = RegistryManager::Global().Get("transform." + pass_name))) {
    }
...
Pass CreateModulePass(const TypedPackedFunc<IRModule(IRModule, PassContext)>& pass_func,
                      int opt_level, String name, Array<String> required, bool traceable) {
...
TVM_REGISTER_GLOBAL("transform.PassInfo")
        .set_body_typed([](int opt_level, String name, Array<String> required, bool traceable) {
...
TVM_STATIC_IR_FUNCTOR(ReprPrinter, vtable)
        .set_dispatch<PassInfoNode>([](const ObjectRef& ref, ReprPrinter* p) {
...
TVM_REGISTER_GLOBAL("transform.MakeModulePass")
        .set_body_typed([](TypedPackedFunc<IRModule(IRModule, PassContext)> pass_func,
...
TVM_REGISTER_GLOBAL("transform.Sequential").set_body([](TVMArgs args, TVMRetValue* ret) {
    Array<Pass> passes = args[0];
    int opt_level = args[1];
    std::string name = args[2];
    Array<String> required = args[3];
