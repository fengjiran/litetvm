...
int FindInsertionPoint(const ScheduleState& self, const Array<Stmt>& subtrees,
    ...
    CHECK(split.last_producer_position < split.first_consumer_position);
    ...
    if (index == -1) {
        ...
        LOG(DFATAL) << "Valid index:(-1, -2, [" << split.last_producer_position + 1 << ", "
...
std::pair<Var, BlockVarDomainInfo> SolveBlockVarDomain(const arith::IntSet& provided,
    ...
    CHECK(var.defined()) << "ValueError: BufferRegion pattern match failed: " << provided_min;
...
void UpdateBlockVarDomainDimwise(
    ...
    for (size_t i = 0; i < ndim; ++i) {
        ...
        if (provided.CanProveSinglePoint(analyzer) && is_const_int(provided.min())) {
            CHECK(required.CanProveSinglePoint(analyzer) &&
        ...
        if (it != iter_doms->end()) {
            ...
            CHECK(analyzer->CanProveEqual(provided.min(), required.min()));
            CHECK(analyzer->CanProveEqual(provided.max(), required.max()));
        }
    }
}
...
Map<Var, arith::IntSet> InverseAffineIterMap(const Array<arith::IterSumExpr>& iter_map,
    ...
    for (const auto& intset: outputs) {
        CHECK(intset.HasLowerBound() && intset.HasUpperBound());
    ...
    for (const auto& kv: rev_min) {
        ...
        CHECK(it != rev_max.end());// InverseAffineIterMap's result vars are assumed stable
...
bool UpdateBlockVarDomainAffine(const BufferNode* buffer, const Array<IterVar>& iter_vars,
    ...
    for (const auto& kv: var_dom) {
        ...
        CHECK(it != var_bound.end());// InverseAffineIterMap's result vars are assumed stable
...
std::vector<BlockVarDomainInfo> CalculateBlockVarDomain(
    ...
    for (const auto& kv: provided_regions) {
        ...
        CHECK_EQ(provided_region.size(), buffer->shape.size());
        CHECK_EQ(required_region.size(), buffer->shape.size());
...
void CalculateProvidedRequiredRegions(
    ...
    for (const StmtSRef& required_block_sref: is_compute_at ? consumer_srefs : producer_srefs) {
        ...
        CHECK(block2realize.count(required_block));
...
private:
    ...
    template<typename>
    friend struct ::litetvm::tir::UnpackedInstTraits;
};
