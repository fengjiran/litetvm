...
void CheckSubtreeCompactDataflow(const ScheduleState& self, const StmtSRef& subtree_root) {
    ...
    public:
        explicit NotCompactDataFlowError(IRModule mod, Stmt subtree_root, Block violate_block,
            ...
            CHECK(subtree_root_->IsInstance<BlockNode>() || subtree_root_->IsInstance<ForNode>());
        }
        String FastErrorString() const final {
...
Map<Var, PrimExpr> GetBindings(const BlockRealize& realize) {
    ...
    CHECK_EQ(all_lhs.size(), all_rhs.size());
...
bool GetVarsTouchedByBlockIters(const BlockRealize& block_realize,
    ...
    CHECK(block_realize->block.same_as(block))
    ...
    CHECK_EQ(block->iter_vars.size(), block_realize->iter_values.size());
...
std::pair<Array<StmtSRef>, std::vector<int>> CollectComputeLocation(const ScheduleState& self,
    ...
    CHECK_LT(lca_pos, loop_srefs.size());
...
std::pair<int64_t, int64_t> GetCumulativeSpaceAndReductionLength(const tir::ScheduleState& self,
   ...
   */
    for (const tir::StmtSRef& loop_sref: loops) {
        ...
        } else if (type == IterVarType::kCommReduce) {
            const int64_t* extent = GetLoopIntExtent(loop_sref);
            if (extent && *extent != -1) {
...
bool NeedsRFactorOrCrossThreadReduction(const tir::ScheduleState& self, //
    ...
    for (const tir::StmtSRef& loop_sref: loops) {
        ...
        if (type != IterVarType::kDataPar && type != IterVarType::kCommReduce) {
            return false;
        }
    }
    ...
    for (size_t i = 0; i < loops.size(); ++i) {
        // Cond 4.
        if (GetLoopIterType(loops[i]) == IterVarType::kCommReduce) {
...
Optional<TensorizeInfo> GetTensorizeLoopMapping(const tir::ScheduleState& self,
    ...
    CHECK(desc_loops.size() == static_cast<size_t>(n_desc_vars));
    CHECK(block_loops.size() == iter_types_block.size());
...
private:
    ...
    void CollectFeasibleSet() {
        ...
        CHECK_EQ(rhs_buffer_index.size(), lhs_buffer_index.size());
        ...
        for (const auto& it: extractor_->rhs_buffer_indices_map_) {
            ...
            CHECK(lhs_buffer_it != extractor_->rhs_buffer_map_.end());
...
TVM_REGISTER_GLOBAL("tir.schedule.GetLoopIterType")
        .set_body_typed([](Schedule sch, LoopRV loop) -> String {
            ...
            if (kind == IterVarType::kDataPar) {
                return "S";
            } else if (kind == IterVarType::kCommReduce) {
                return "R";
            } else {
                return "O";
            }
