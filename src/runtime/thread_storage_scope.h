...
#ifndef LITETVM_RUNTIME_THREAD_STORAGE_SCOPE_H
#define LITETVM_RUNTIME_THREAD_STORAGE_SCOPE_H
    ...
    void Init(size_t base, const std::vector<std::string>& launch_param_tags) {
        ...
        for (size_t i = 0; i < launch_param_tags.size(); ++i) {
            ...
            if (tag == launch_param::kUseDynamicSharedMemoryTag) {
                CHECK_EQ(i, launch_param_tags.size() - 1)
...
namespace std {
template<>
struct hash<::litetvm::runtime::StorageScope> {
    std::size_t operator()(const ::litetvm::runtime::StorageScope& k) const {
        return static_cast<size_t>(k.rank);
    }
};
}// namespace std
#endif//LITETVM_RUNTIME_THREAD_STORAGE_SCOPE_H
