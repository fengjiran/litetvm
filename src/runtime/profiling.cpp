...
public:
    virtual void Start() {
        TVMSynchronize(static_cast<int>(device_.device_type), device_.device_id, nullptr);
...
Timer Timer::Start(Device dev) {
    auto f = RegistryManager::Global().Get(std::string("profiling.timer.") +
                                           DLDeviceType2Str(static_cast<int>(dev.device_type)));
    if (f == nullptr) {
        {
            std::lock_guard<std::mutex> lock(seen_devices_lock);
            if (seen_devices.find(dev.device_type) == seen_devices.end()) {
                LOG(WARNING)
                        << "No timer implementation for " << DLDeviceType2Str(static_cast<int>(dev.device_type))
...
ObjectRef AggregateMetric(const std::vector<ObjectRef>& metrics) {
    CHECK_GT(metrics.size(), 0) << "Must pass a non-zero number of metrics";
...
std::string DeviceString(Device dev) {
    return DLDeviceType2Str(static_cast<int>(dev.device_type)) + std::to_string(dev.device_id);
}
...
Map<String, ObjectRef> parse_metrics(dmlc::JSONReader* reader) {
    ...
    while (reader->NextObjectItem(&metric_name)) {
        ...
        CHECK(!reader->NextObjectItem(&metric_value_name));
...
PackedFunc WrapTimeEvaluator(PackedFunc pf, Device dev, int number, int repeat, int min_repeat_ms,
    ...
    if (static_cast<int>(dev.device_type) == static_cast<int>(TVMDeviceExtType::kDLMicroDev)) {
        auto get_micro_time_evaluator = RegistryManager::Global().Get("micro._GetMicroTimeEvaluator");
    ...
    auto ftimer = [pf, dev, number, repeat, min_repeat_ms, limit_zero_time_iterations,
        ...
        if (cache_flush_bytes > 0) {
            arr1 = NDArray::Empty({cache_flush_bytes / 4}, {static_cast<int>(DLDataTypeCode::kDLInt), 32, 1}, dev);
            arr2 = NDArray::Empty({cache_flush_bytes / 4}, {static_cast<int>(DLDataTypeCode::kDLInt), 32, 1}, dev);
        }
