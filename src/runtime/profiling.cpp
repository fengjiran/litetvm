...
Timer DefaultTimer(Device dev) {
    return Timer(make_object<DefaultTimerNode>(dev));
}
...
Timer Timer::Start(Device dev) {
    auto f = litetvm::ffi::Function::GetGlobal(std::string("profiling.timer.") +
...
std::vector<int64_t> ToShape(NDArray shape_tensor) {
    ...
    if (dtype.is_int() && dtype.bits() == 32 && dtype.lanes() == 1) {
        auto* dims = reinterpret_cast<int32_t*>(dl_tensor->data);
        shape.assign(dims, dims + ndim);
    } else if (dtype.is_int() && dtype.bits() == 64 && dtype.lanes() == 1) {
        auto* dims = reinterpret_cast<int64_t*>(dl_tensor->data);
        shape.assign(dims, dims + ndim);
    } else {
...
String ReportNode::AsTable(bool sort, bool aggregate, bool compute_col_sums) const {
    ...
    std::vector<Map<String, Any>> aggregated_calls;
    if (aggregate) {
...
Report::Report(Array<Map<String, Any>> calls,
...
Map<String, Any> parse_metrics(dmlc::JSONReader* reader) {
    reader->BeginObject();
