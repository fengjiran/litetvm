...
struct StringObjTrait {
    ...
    static bool SEqualReduce(const StringObj* lhs, const StringObj* rhs,
                             SEqualReducer equal) {
...
struct RefToObjectPtr : ObjectRef {
...
TVM_REGISTER_REFLECTION_VTABLE(runtime::StringObj, StringObjTrait)
        .set_creator([](const std::string& bytes) {
            return RefToObjectPtr::Get(String(bytes));
        })
        .set_repr_bytes([](const Object* n) -> std::string {
            return GetRef<String>(static_cast<const StringObj*>(n))
                    .
                    operator std::string();
        });

TVM_STATIC_IR_FUNCTOR(ReprPrinter, vtable)
        .set_dispatch<StringObj>([](const ObjectRef& node, ReprPrinter* p) {
            auto* op = static_cast<const StringObj*>(node.get());
...
void NDArrayHash(const NDArray::Container* arr, SHashReducer* hash_reduce,
    ...
    if (hash_data) {
        (*hash_reduce)
                ->SHashReduceHashedValue(String::StableHashBytes(
                        static_cast<const char*>(arr->dl_tensor.data), runtime::GetDataSize(arr->dl_tensor)));
    }
}

void NDArrayContainerTrait::SHashReduce(const NDArray::Container* key,
...
TVM_REGISTER_REFLECTION_VTABLE(runtime::NDArray::Container, NDArrayContainerTrait)
        ...
        .set_repr_bytes([](const Object* n) -> std::string {
            ...
            const auto* ndarray = static_cast<const NDArray::Container*>(n);
...
TVM_REGISTER_REFLECTION_VTABLE(ArrayNode, ArrayNodeTrait)
        .set_creator([](const std::string&) -> ObjectPtr<Object> {
            return make_object<ArrayNode>();
        });

struct ShapeTupleObjTrait {
    static constexpr std::nullptr_t VisitAttrs = nullptr;

    static void SHashReduce(const ShapeTupleNode* self, SHashReducer hash_reduce) {
...
TVM_REGISTER_REFLECTION_VTABLE(ShapeTupleNode, ShapeTupleObjTrait)
        .set_creator([](const std::string& blob) {
        ...
        .set_repr_bytes([](const Object* n) -> std::string {
            ...
            dmlc::MemoryStringStream mstrm(&blob);
            support::Base64OutStream b64strm(&mstrm);
            const auto* shape = static_cast<const runtime::ShapeTupleNode*>(n);
