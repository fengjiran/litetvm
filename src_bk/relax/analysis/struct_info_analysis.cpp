...
#ifndef LITETVM_RELAX_STRUCT_INFO_ANALYSIS_H
#define LITETVM_RELAX_STRUCT_INFO_ANALYSIS_H
...
public:
    ...
    // module: distributed
    Type VisitStructInfo_(const distributed::DTensorStructInfoNode* op) final {
        return ObjectType();
    }
    ...
    Type VisitStructInfo_(const FuncStructInfoNode* op) final {
        if (op->IsOpaque()) return PackedFuncType();
        Array<Type> params = op->params.value().Map([this](const StructInfo& sinfo) {
            return this->VisitStructInfo(sinfo);
        });
...
StructInfo StructInfoFromType(const Type& type) {
    ...
    if (const auto* prim_type = type.as<PrimTypeNode>()) {
        return PrimStructInfo(prim_type->dtype);
    }

    if (const auto* shape_type = type.as<ShapeTypeNode>()) {
        return ShapeStructInfo(shape_type->ndim);
    }
    ...
    if (const auto* func_type = type.as<FuncTypeNode>()) {
        Array<StructInfo> params = func_type->arg_types.Map([](const Type& param) {
            return StructInfoFromType(param);
        });
        StructInfo ret = StructInfoFromType(func_type->ret_type);
        // TODO(relax-team): Maybe add purity into the type as well
        return FuncStructInfo(params, ret, true);
    }
...
public:
    WellDefinedEraser(std::function<Optional<PrimExpr>(const tir::Var& var)> f_shape_var_map,
        ...
        : f_shape_var_map_(std::move(f_shape_var_map)), f_var_map_(std::move(f_var_map)), ana_(ana) {}
    ...
    StructInfo VisitStructInfo_(const PrimStructInfoNode* op) final {
        ...
        if (!has_undefined) {
            ...
            return PrimStructInfo(value.value());
        }
        return PrimStructInfo(op->dtype);
    }
    ...
    StructInfo VisitStructInfo_(const ShapeStructInfoNode* op) final {
        ...
        if (op->values.defined()) {
            std::swap(has_undefined_, has_undefined);
            values = op->values.value().Map([&](PrimExpr val) {
                return this->VisitPrimExpr(val);
            });
            std::swap(has_undefined_, has_undefined);
        }
        // erase symbolic shape if we have undefined.
        ...
        return ShapeStructInfo(op->ndim);
    }
    ...
    StructInfo VisitStructInfo_(const TensorStructInfoNode* op) final {
        ...
        if (!has_undefined) {
            ...
            return TensorStructInfo(op->dtype, op->ndim, vdev);
        }
        return TensorStructInfo(op->dtype, op->ndim, vdev);
    }
    ...
    PrimExpr VisitPrimExpr(const PrimExpr& expr) {
        ...
        PrimExpr val = ExprMutator::VisitExpr(expr);
        if (!val.same_as(expr)) {
    ...
    Expr VisitExpr_(const VarNode* var) final {
        ...
        if (ret.defined()) {
            CHECK(ret.as<VarNode>() || ret.as<ShapeExprNode>())
    ...
    PrimExpr VisitExpr_(const tir::VarNode* var) final {
        ...
        if (ret.defined()) {
            ...
            CHECK(value.dtype() == DataType::Int(64)) << "Can only provide i64 expressions in shape";
            return value;
        }
...
StructInfo EraseToWellDefined(
        const StructInfo& info, const std::function<Optional<PrimExpr>(const tir::Var& var)>& f_shape_var_map,
        const std::function<Optional<Expr>(const Var& var)>& f_var_map, arith::Analyzer* ana) {
    if (ana == nullptr) {
        arith::Analyzer inst;
...
public:
    ...
    StructInfo Derive(const FuncStructInfo& finfo, const Call& call, const BlockBuilder& ctx) {
        ...
        CHECK(params.size() == call->args.size())
        ...
        for (size_t i = 0; i < params.size(); ++i) {
            ...
            CHECK(!(res == BaseCheckResult::kFailL0 || res == BaseCheckResult::kFailL1))
...
protected:
    ...
    BaseCheckResult ShapeMatchCheck(const Expr& lhs, const Expr& rhs) final {
        ...
        auto lhs_shape = lhs.as<ShapeExprNode>();
        auto rhs_shape = rhs.as<ShapeExprNode>();
        CHECK(lhs_shape) << "lhs must have a shape";
...
public:
    ...
    StructInfo VisitStructInfo_(const PrimStructInfoNode* lhs, const StructInfo& other) final {
        auto* rhs = other.as<PrimStructInfoNode>();
        if (rhs == nullptr) return ObjectStructInfo();
        if (lhs->dtype != rhs->dtype) {
        ...
        if (!lhs->value.defined() || !rhs->value.defined() ||
            ...
            if (!lhs->value.defined()) {
                ...
                return PrimStructInfo(lhs->dtype);
            }
        }
    ...
    StructInfo VisitStructInfo_(const TupleStructInfoNode* lhs, const StructInfo& other) final {
        ...
        if (!fields.defined()) return ObjectStructInfo();

        // same length tuple.
        if (!fields.same_as(lhs->fields)) {
            return TupleStructInfo(fields.value());
        }
        return GetRef<StructInfo>(lhs);
    }
    ...
    StructInfo VisitStructInfo_(const FuncStructInfoNode* lhs, const StructInfo& other) final {
        ...
        return FuncStructInfo(params.value(), ret, purity);
    }
...
class SymbolicVarCollector : public ExprVisitor,
                             public StructInfoVisitor,
...
private:
    ...
    void VisitExpr_(const FunctionNode* op) final {
        WithMode(VisitMode::kProvideDefinition, [&]() {
            for (Var param: op->params) {
                StructInfoVisitor::VisitStructInfo(GetStructInfo(param));
            }
        });
    ...
    void VisitBinding_(const MatchCastNode* binding) final {
        WithMode(VisitMode(static_cast<int>(VisitMode::kProvideDefinition) | static_cast<int>(VisitMode::kRequireDefinition)),
    ...
    void VisitStructInfoExprField(const PrimExpr& expr) final {
        if (static_cast<int>(mode_) & static_cast<int>(VisitMode::kProvideDefinition)) {
            if (auto var = expr.as<tir::Var>()) {
                defined_symbolic_var_.insert(var.value());
            }
        }
        if (static_cast<int>(mode_) & static_cast<int>(VisitMode::kRequireDefinition)) {
...
#endif//LITETVM_RELAX_STRUCT_INFO_ANALYSIS_H
