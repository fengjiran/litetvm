...
bool ProducerCoversConsumer(const Array<PrimExpr>& buffer_shape,
                            ...
                            arith::Analyzer* analyzer) {
    CHECK_EQ(buffer_shape.size(), consumed_region.size());
    CHECK_EQ(produced_region.size(), consumed_region.size());
...
LITETVM_API Array<Bool> GetCachedFlags(const ScheduleState& self, const StmtSRef& block_sref) {
