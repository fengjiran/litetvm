...
void LeafBlockRemovalPlan(const ScheduleState& self, const StmtSRef& leaf_block_sref,
    ...
    CHECK(sref != nullptr && sref->stmt != nullptr);
...
Optional<LoopRV> TileWithTensorIntrin(const tir::Schedule& sch, const tir::BlockRV& block_rv,
    ...
    if (info->block_iter_paddings.defined()) {
        ...
        for (const auto& producer: sch->GetProducers(block_rv)) {
            ...
            CHECK_EQ(the_original_producers.size(), 1u);
            auto the_original_producer = the_original_producers[0];
            CHECK(original_producers.count(sch->GetSRef(the_original_producer).get()));
        ...
        for (const auto& consumer: sch->GetConsumers(block_rv)) {
            ...
            CHECK_EQ(the_original_consumers.size(), 1u);
            auto the_original_consumer = the_original_consumers[0];
            CHECK(original_consumers.count(sch->GetSRef(the_original_consumer).get()));
