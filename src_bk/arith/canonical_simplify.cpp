...
inline PrimExpr ModImpl(PrimExpr a, PrimExpr b, DivMode mode) {
    ...
    CHECK_EQ(static_cast<int>(mode), static_cast<int>(DivMode::kFloorDiv));
    return floormod(a, b);
}
...
public:
   ...
   */
    bool CanPushCastToChildren(DataType dtype, Analyzer* analyzer) const {
        ...
        if (this->scale != 1) {
            CHECK(!this->dtype.is_uint() || this->scale > 0);
...
TVM_STATIC_IR_FUNCTOR(ReprPrinter, vtable)
        .set_dispatch<SplitExprNode>([](const ObjectRef& node, ReprPrinter* p) {
            ...
            switch (op->div_mode) {
                ...
                case DivMode::kTruncDiv:
                    p->stream << "truncdiv";
                    break;
                case DivMode::kFloorDiv:
                    p->stream << "floordiv";
                    break;
            }
            p->stream << ')';
