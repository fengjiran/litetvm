...
public:
   ...
   */
    LITETVM_API AllocateConst(Var buffer_var, DataType dtype, Array<PrimExpr> extents,
...
public:
    LITETVM_API DeclBuffer(Buffer buffer, Stmt body);
...
public:
    LITETVM_API IfThenElse(PrimExpr condition, Stmt then_case, Optional<Stmt> else_case = NullOpt);
...
public:
    LITETVM_API For(Var loop_var, PrimExpr min, PrimExpr extent, ForKind kind, Stmt body,
...
public:
    LITETVM_API While(PrimExpr condition, Stmt body);
...
public:
    ...
    PrefetchNode(Buffer buffer, Array<Range> bounds)
        : StmtNode(), buffer(buffer), bounds(bounds) {}
...
public:
    LITETVM_API explicit Prefetch(Buffer buffer, Array<Range> bounds);
...
public:
    LITETVM_API explicit BufferRegion(Buffer buffer, Array<Range> region);
   ...
   */
    LITETVM_API static BufferRegion FullRegion(Buffer buffer);
   ...
   */
    LITETVM_API static BufferRegion FromPoint(Buffer buffer, Array<PrimExpr> indices);
...
public:
    LITETVM_API explicit MatchBufferRegion(Buffer buffer, BufferRegion source);
...
public:
    LITETVM_API explicit Block(Array<IterVar> iter_vars, Array<BufferRegion> reads,
...
public:
    LITETVM_API explicit BlockRealize(Array<PrimExpr> iter_values, PrimExpr predicate, Block block);
 ...
 */
LITETVM_API PrimExpr TypeAnnotation(DataType dtype);

// overload printing of for type.
LITETVM_API std::ostream& operator<<(std::ostream& os, ForKind kind);
