...
#ifndef LITETVM_TARGET_REGISTRY_H
#define LITETVM_TARGET_REGISTRY_H
...
#define DEFINE_GET_LOWER_FUNC_(OP)                                                                           \
    inline const runtime::PackedFunc* Get##OP##LowerFunc(const std::string& target,                          \
                                                         uint8_t type_code) {                                \
        return runtime::RegistryManager::Global().Get("tvm.datatype.lower." + target + "." #OP "." +         \
...
#endif//LITETVM_TARGET_REGISTRY_H
