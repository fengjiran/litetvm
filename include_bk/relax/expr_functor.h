...
#ifndef LITETVM_RELAX_EXPR_FUNCTOR_H
#define LITETVM_RELAX_EXPR_FUNCTOR_H
...
public:
   ...
   */
    virtual R VisitExpr(const Expr& n, Args... args) {
        CHECK(n.defined()) << "Found null pointer node while traversing AST. The previous pass may "
...
private:
    ...
    using VisitBindingVTable = NodeFunctor<void(const ObjectRef& n, ExprVisitor* self, const VarBindingNode* binding)>;
    // initialize the vtable.
...
protected:
   ...
   */
    bool VisitAndCheckStructInfoFieldUnchanged(const ObjectRef& struct_info) {
        if (const auto* sinfo = struct_info.as<StructInfoNode>()) {
...
private:
    ...
    using VisitBindingVTable = NodeFunctor<void(const ObjectRef& n, ExprMutator* self, const VarBindingNode* binding)>;
    // initialize the vtable.
    static VisitBindingVTable InitVisitBindingVTable();
};
...
#endif//LITETVM_RELAX_EXPR_FUNCTOR_H
